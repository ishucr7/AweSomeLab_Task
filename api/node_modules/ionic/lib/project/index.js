...
function determineProjectDetails(rootProjectDir, projectName, projectConfig, deps) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        let name;
        let type;
        if (projectName) {
            name = projectName;
            debug(`Project name from projectName: ${chalk_1.default.bold(name)}`);
        }
        ...
        if (!type) {
            for (const projectType of constants_1.PROJECT_TYPES) {
                const p = yield createProjectFromType(path.resolve(rootProjectDir, constants_1.PROJECT_FILE), projectName, deps, projectType);
                if (yield p.detected()) {
                    debug(`Project type from detection: ${chalk_1.default.bold(prettyProjectName(p.type))} ${p.type ? chalk_1.default.bold(`(${p.type})`) : ''}`);
                    type = p.type;
                    break;
                }
            }
        ...
        throw new Error(`Could not determine project type (project config: ${chalk_1.default.bold(format_1.prettyPath(path.resolve(rootProjectDir, constants_1.PROJECT_FILE)))}).\n` +
...
exports.determineProjectDetails = determineProjectDetails;
